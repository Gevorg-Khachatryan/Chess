...
class Chess:
    ...
    def click(self, event):
        ...
        if 0 < event.x // 30 < 9 and 0 < event.y // 30 < 9 and (event.x // 30 + event.y // 30)%2==0:
            if [event.x // 30, event.y // 30] in self.w_pawn and self.clc == False:
                ...
                self.y = event.y // 30
                self.clc = True
            if self.clc and [event.x // 30, event.y // 30] not in self.w_pawn and [event.x // 30, event.y // 30] not in self.b_pawn:
    ...
    def pc(self):
        for i in range(len(self.b_pawn)):
            ...
            y-=1
            x-=randrange(-1,2,2)
            print([self.b_pawn[i][0],self.b_pawn[i][1]],[x,y],[x-(self.b_pawn[i][0]-x),y-(self.b_pawn[i][0]-y)])
            if 0 < x  < 9 and 0 < y < 9 and (x +y) % 2 == 0 :
                if [x,y]not in self.w_pawn and [x,y]not in self.b_pawn:
                    self.b_pawn[i][0]=x
                    self.b_pawn[i][1]=y
                    break
                if [x,y] in self.w_pawn and [x-(self.b_pawn[i][0]-x),y-(self.b_pawn[i][1]-y)] not in self.w_pawn\
                        and [x-(self.b_pawn[i][0]-x),y-(self.b_pawn[i][0]-y)] not in self.b_pawn\
                        and 0<x-(self.b_pawn[i][0]-x)<9 and 0<y-(self.b_pawn[i][0]-y)<9:
                    self.b_pawn[i][0] = x-(self.b_pawn[i][0]-x)
                    self.b_pawn[i][1] = y-(self.b_pawn[i][0]-y)
                    t=0
                    while t<12:
